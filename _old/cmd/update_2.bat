...
bin\bg.exe fcprint 15 0 %bordercolor% "███████████████████████████████████████████████████████"
bin\bg.exe fcprint 16 0 %bordercolor% "███████████████████████████████████████████████████████"
bin\bg.exe fcprint 17 0 0F "───────────────────────────────────────────────────────"
chcp %_codepage% > nul
goto :eof
:drawbuttons
...
if exist "cmd\settings.bat" call cmd\settings.bat modlist >>data\update.log 2>&1
echo.Finalizing Update...
if "%lite%"=="1" if exist "cmd\settings.bat" call cmd\settings.bat lite 1 >>data\update.log 2>&1
...
del /f /q "data\pages\modlist.*" 2>nul
echo.Restarting PCMod...
ping 127.0.0.1 -n 3 >nul
del "bin\.cancelLaunch" 2>nul
start "" "PCMod.exe" --cleanup-old cmd
exit
...
call :update.install.launcher.v2
goto :eof

:update.install.launcher.v2
echo.Installing Launcher Updates (v2)...
echo a | xcopy /e /v /y "data\update\%download%\*" .\ >>data\update.log 2>&1
...
set state=%1
set lltype=%2
if "%lltype%"=="launcher" set llupdate=%launcher_version% to %launcher_update%
if "%lltype%"=="pack" set llupdate=%pack%/%pack_version% to %pack_update%
if "%lltype%"=="both" set llupdate=BOTH %pack%/%pack_update% + %launcher_update%
if "%lltype%"=="empty" set llupdate=%pack%/%pack_update% + %launcher_update%
set modcnt=0
